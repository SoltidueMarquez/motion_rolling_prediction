...
def load_data(
    ...
    # Processed motion files are trusted local artifacts generated by
    # prepare_data.py. PyTorch 2.6+ defaults to weights_only=True, which cannot
    # deserialize their NumPy metadata and SMPLModelType enum.
    ...
    if (dataset_path / mean_path).exists() and (dataset_path / std_path).exists():
        mean = torch.load(os.path.join(dataset_path, mean_path), weights_only=True)
        std = torch.load(os.path.join(dataset_path, std_path), weights_only=True)
